...
class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_loadMoreActions_requestMoreFromLoader() {
        ...
        sut.simulateLoadMoreFeedAction()
        XCTAssertEqual(loader.loadMoreCallCount, 3, "Expected no request after loaing all pages")
    }
    ...
    func test_loadingFeedIndicator_isVisibleWhileLoadingFeed() {
        ...
        XCTAssertFalse(sut.isShowingLoadingIndicator, "Expected no loading indicator once user initiated loader completes with error")
    }
    ...
    func test_feedImageView_loadImageURLWhenVisible() {
        ...
        XCTAssertEqual(loader.loadedImageURLs, [], "Expected no imageURL requests until views become visible")
    ...
    func test_feedImageViewLoadingIndicator_isVisibleWhileLoadingImage() {
        ...
        XCTAssertEqual(view1?.isShowingImageLoadingIndicator, true, "Expected no loading indicator state change for second view once first image loading comopletes successfully")
    ...
    func test_feedImageView_rendersImageLoadedFromURL() {
        ...
        XCTAssertEqual(view1?.renderedImage, .none, "Expected no image state change for second view once first image loading comopletes successfully")
    ...
    func test_feedImageViewRetryButton_isVisibleOnImageURLLoadError() {
        ...
        let imageData0 = UIImage.make(withColor: .red).pngData()!
        loader.completeImageLoading(with: imageData0, at: 0)
        XCTAssertEqual(view0?.isShowingRetryAction, false, "Expected no retry action for frist view once first image loading completes successfully")
        XCTAssertEqual(view1?.isShowingRetryAction, false, "Expected no retry action state change for second view once first image loading comopletes successfully")
        
        loader.completeImageLoadingWithError(at: 1)
        XCTAssertEqual(view0?.isShowingRetryAction, false, "Expected no retry action state change for frist view once second image loading completes with error")
        XCTAssertEqual(view1?.isShowingRetryAction, true, "Expected retry action for seconde view once second image loading completes with error")
    }
    ...
    func test_feedImageViewRetryButton_isVisibleOnInvalidImageData() {
        ...
        XCTAssertEqual(view?.isShowingRetryAction, false, "Expected no retry action while loading first image")
    ...
    func test_feedImageView_cancelsImageURLPreloadingWhenNotNearVisibleAnymore() {
        ...
        XCTAssertEqual(loader.cancelledImageURLs, [image0.url], "Expected first cancelled image URL requests once image is not near visible anymore")
    ...
    func test_feedImageView_doesNotLoadImageAgainUntilPreviousRequestCompletes() {
        ...
        sut.simulateFeedImageViewVisible(at: 0)
        XCTAssertEqual(loader.loadedImageURLs, [image.url], "Expected no reuqest until previous completes")
        ...
        sut.simulateFeedImageViewNotVisible(at: 0)
        sut.simulateFeedImageViewVisible(at: 0)
        XCTAssertEqual(loader.loadedImageURLs, [image.url, image.url, image.url], "Expected third request when visble after canceling previous complete")
